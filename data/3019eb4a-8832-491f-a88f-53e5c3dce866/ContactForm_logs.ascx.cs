...
{
    public partial class ContactForm_logs : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {

            string XMLfilePath = Server.MapPath("~/data/contact_form_log.xml");
            string XSLTfilePath = Server.MapPath("~/data/form_log.xslt");

            //Check if XML file exists
            if (!File.Exists(XMLfilePath))
            {
                xsltResultsDiv.InnerHtml = "<div class='notice'><h2>Warning</h2><p>The contact_form_log.xml file does not containt any logs or does not exist.</p></div>";
                return;
            }
            else
            {
