...
<xsl:stylesheet version="1.0"
  ...
  XSLTsearch.xslt
  ======================================================================
  Copyright 2006-2007 Percipient Studios. All rights reserved.
  CPL License (http://www.opensource.org/licenses/cpl1.0.txt).
  Commercial licenses available.

  Version 2.7
  Tested with Umbraco version 2.1 through 2.1.7, and 3.0RC1 through 3.0.2.
  ...
  -->
	...
	<xsl:variable name="XSLTsearchVersion" select="'2.7'"/>
	...
	<xsl:template match="/">
		<xsl:choose>
			...
			<xsl:when test="number($source)=$currentID">
				...
				<xsl:call-template name="search">
					<xsl:with-param name="items" select="$currentPage/ancestor-or-self::node [@level = 1]"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template name="pageNumbers">
		...
		<xsl:choose>
			...
			<xsl:when test="($distanceFromCurrent &gt; -10 and $distanceFromCurrent &lt; 10)">
				<a>
					<xsl:attribute name="href">
						?search=<xsl:value-of select="$search"/>&amp;page=<xsl:value-of select="$pageIndex"/>
					</xsl:attribute>
					<xsl:value-of select="$pageIndex"/>
				</a>&nbsp;
			</xsl:when>
